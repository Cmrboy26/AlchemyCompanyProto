...
public class GameScreen implements Screen {
    ...
    private void prepareUI() {
        ...
        Label key = new Label("Excess Prod : Stored, Next Remove Storage", skin);
        ...
        for (final Resource r : Resource.values()) {
            Label counter = new Label("- : -", skin) {
                ...
                public void act(float delta) {
                    ...
                    float stored = player.currentStoredResources.getOrDefault(r, 0f);
                    float toBeUsed = player.calculatedNetStoredResourceNextTurn.getOrDefault(r, 0f);
                    setText(production + " : " + stored + ", " + toBeUsed);
                    super.act(delta);
                }
            };
